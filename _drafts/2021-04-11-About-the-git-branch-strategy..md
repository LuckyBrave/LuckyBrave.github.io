...
그렇게 도입된 코드 리뷰를 할때 동료의 런타임 오류도 잡아주고 서로의 코드에 대해 코멘트하며 성장하는 느낌과 성취감을 느낄 수 있었다.  
...
실력은 둘째치더라도 2000년대 중반까지만 해도 우리는 철야하고 다음날 아침에 팀장이 사준 도기니탕을 특근 수당으로 받고  
다시 회사로 들어가 라꾸라꾸도 없이 잠깐 눈을 붙인 후 일어나 다시 개발을 할 정도로 열정과 의지가 있었다.  
그런데 왜 우리의 코드 리뷰는 항상 실패할까?   
늙어서 그런건가? 영향이 없다고 할 수는 없지만, 그렇다면 들어온지 얼마안된 신입은 왜 힘들어 하는 걸까?  
남의 코드를 보는 게 힘든 걸까? 물론 힘들지만 우린 늘 남의 코드를 봐왔다.  
내 일이 아니라는 인식 때문인가? 아직 이런 개발 문화가 익숙하지 않아서 일까?  
그럴 수도 있다고 생각했다.  
...
다르지만 어찌됐든 Git flow는 웹 서비스에 적합하지 않다고 반성 노트에 남겼다.   
...
그럼 Git flow가 문제였을까? 나는 사실 그러길 바랐다. Git flow가 문제라면 flow를 다른 걸로 바꾸기만 하면 해결될테니깐.  
하지만 계속되는 점검에 결국 우리가 가지고 있는 문제들은 Git flow의 문제라고 보기 어려웠다.  
웹 서비스와 맞지 않는 Git flow를 선택한 약간의 부작용은 있었겠지만 근본적으로 Git flow의 문제가 아닌 건 확실했다.  

첫번째 문제는 우리의 문제 의식이였다.  
단지 대중적으로 사용된다고 해서 고민과 조사없이 우리가 운영하는 소프트웨어 유형과 적합하지 않는 Git flow를 그대로 가져와 사용했다.  
...
메이저 feature 브랜치 하나에는 다음에 배포될 모든 기능들이 담겨져 있었다.   
그런 방대한 양을 과연 매번 코드 리뷰할 수 있을까?  
물론 Git flow 사상을 따르다 보니 feature 브랜치의 life cycle이 커진것도 있었지만 이건 어디까지나 우리가 조절할 수 있는 문제였다.

그외에도 여러 문제가 있지만(CI/CD등) 가장 큰 문제는 역시 우리의 문제 의식이였다.

## 결국 우리에게 맞는 Git 브랜치 전략을 설계해야 된다.
우린 뒤늦게 기술 부채를 청산하려고 현재 노력 중이다.  
이미 검증된 전략들의 장점을 충분히 검토해서 취하고 우리에게 불필요한 과정은 생략하며 찾아가고 있다.  
**Vincent Driessen**의 말처럼 모든 걸 해결해 줄 수 있는 만병통치약은 없다.  
우리 스스로 판단하고 결정해야 된다.  

우리가 겪는 여러 문제들이 있지만 우선 이 정도 결론은 내릴 수 있었다.
...
병합 전략은 히스토리 정리를 위해 **rebase merge**로 한다. 이거 역시 필수로 한다.

우선 목표에 도달하기 위해 언급한 세 가지 규칙을 실행하는 것부터 시작한다.
...
대다수의 잘하고 있는 개발팀말고 우리처럼 준비되지 않아 고생하는 소수의 순진한 개발팀을 위해 작성한다.
