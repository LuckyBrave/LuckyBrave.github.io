...
5년 전 우리팀은 VCS를 SVN으로 사용하고 있었다.   
각각의 머지할 파일들을 개발자 각자 관리하면서 코드관리에 대한 리소스가 커지는 상황이었다.
다양한 오픈소스 진영들이 Github을 기반으로 활발해지던 시기였고,
개발자 컨퍼런스에서도 너도 나도 git도입을 이야기 하던 시기였다.
도입을 하고 나서도, 한동안 Git을 SVN처럼 사용했다.
하지만 다른 팀처럼 Git 자체를 도입했다는 생각에 뿌듯했다.
...
==> 릴리즈 브랜치 기반의 배포, 태깅 기반 아님(기존과 사상과 다른부분들..)
